...
public class PoligonalLiquid extends Liquid {
    public Color colorFrom = Color.white.cpy(), colorTo = Color.white.cpy();
    public int cells = 18;

    public PoligonalLiquid(String name, Color color) {
        super(name, color);
    }
    ...
    public void drawPuddle(Puddle puddle) {
        ...
        Tmp.c1.set(color).shiftValue(-0.05f);
        Draw.color(Tmp.c1);
        ...
        for (int i = 0; i < cells; i++) {
            ...
            float angle = Mathf.absin(Time.time + ((i + id) % 60) * 54, 75f * rand.random(1f, 2f), 1f);
            Fill.poly(vx + Mathf.sin(Time.time + i * 53, sscl, smag), vy + Mathf.sin(Time.time + i * 3, sscl, smag), 6, f * 3.8f * rand.random(0.2f, 1f) * angle);
        }
