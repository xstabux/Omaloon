...
public class MultiCrafter extends Block {
                        ...
                        table.table(Tex.underline, plan -> {
                            plan.table(input -> {
                                ...
                                for (ItemStack stack : craft.consumeItems) input.add(new Table(){{
                                    ...
                                    add(Strings.autoFixed(stack.amount / (craft.craftTime / 60f), 2) + StatUnit.perSecond.localized()).padLeft(2).padRight(5).color(Color.lightGray).style(Styles.outlineLabel);
                                }}).pad(5f);
    ...
    public void addCraftBars(Craft craft) {
        for (LiquidStack stack : craft.consumeLiquids) addLiquidBar(stack.liquid);
        for (LiquidStack stack : craft.outputLiquids) addLiquidBar(stack.liquid);
    }
    ...
    public class MultiCrafterBuild extends Building {
        ...
        public @Nullable Craft getCraft() {return currentPlan == -1 ? null : crafts.get(currentPlan);}
        public float getPowerCons() {return getCraft() != null ? getCraft().consumePower : 0f;}
        public LiquidStack[] getLiquidCons() {return getCraft() != null ? getCraft().consumeLiquids : LiquidStack.empty;}
        ...
        @Override public Integer config() {return currentPlan;}
        ...
        public boolean acceptItem(Building source, Item item) {
            if (getCraft() == null) return false;
            return Structs.contains(getCraft().consumeItems, stack -> stack.item == item && items.get(item) < stack.amount * itemCapacity);
        }
        @Override
        public boolean acceptLiquid(Building source, Liquid liquid) {
            if (getCraft() == null) return false;
            return liquids.get(liquid) < block.liquidCapacity && Structs.contains(getCraft().consumeLiquids, stack -> stack.liquid == liquid);
        }

        @Override
        public boolean shouldConsume() {
            if (getCraft() == null) return false;
            for (ItemStack stack : getCraft().outputItems) if (items.get(stack.item) >= getMaximumAccepted(stack.item)) return false;
            for (LiquidStack stack : getCraft().outputLiquids) if (liquids.get(stack.liquid) >= block.liquidCapacity) return false;
            return enabled;
        }
        ...
        public void updateTile() {
            if (efficiency > 0 && getCraft() != null) {
                ...
                if (wasVisible && Mathf.chance(getCraft().updateEffectChance)) getCraft().updateEffect.at(x + Mathf.range(size * 4f), y + Mathf.range(size * 4f));
                for(LiquidStack output : getCraft().outputLiquids) handleLiquid(this, output.liquid, Math.min(output.amount * getProgressIncrease(1f), liquidCapacity - liquids.get(output.liquid)));
                if (progress >= 1f) {
                    progress %= 1f;
                    consume();
                    if (wasVisible) getCraft().craftEffect.at(x, y);
                    if (getCraft() != null) for (ItemStack out : getCraft().outputItems) for (int i = 0; i < out.amount; i++) offload(out.item);
                }
            } else {
        ...
        public void dumpOutputs() {
            if(getCraft() != null && timer(timerDump, dumpTime / timeScale)){
                if (getCraft().outputItems == null) return;
                for(ItemStack output : getCraft().outputItems){
                    dump(output.item);
                }

                if (getCraft().outputLiquids == null) return;
