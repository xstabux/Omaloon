...
public class CustomCategory  {
    ...
    public static void loadIcons() {
        for(Entry<String, TextureRegionDrawable> entry : Icon.icons){
    ...
    public void loadEvent() {
        Events.on(EventType.WorldLoadEvent.class, event -> {
            Core.app.post(() -> {
                currentCategory = OlCategory.distribution;
    ...
    public OlCategory currentCategory = OlCategory.distribution;

    Seq<Block> returnArray = new Seq<>(), returnArray2 = new Seq<>();
    Seq<OlCategory> returnCatArray = new Seq<>();
    boolean[] categoryEmpty = new boolean[OlCategory.all.size];
    ObjectMap<OlCategory,Block> selectedBlocks = new ObjectMap<>();
    ObjectFloatMap<OlCategory> scrollPositions = new ObjectFloatMap<>();
    ...
    boolean gridUpdate(InputHandler input){
        ...
        if(Core.input.keyTap(Binding.pick) && player.isBuilder() && !Core.scene.hasDialog()) { //mouse eyedropper select
            ...
            if(tryRecipe != null && tryRecipe.isVisible() && unlocked(tryRecipe)){
                ...
                currentCategory = OlCategory.of(input.block.category);
                return true;
            }
        }
        ...
        for(int i = 0; i < blockSelect.length; i++){
            if(Core.input.keyTap(blockSelect[i])){
                if(i > 9){ //select block directionally
                    ...
                    OlCategory category = OlCategory.all.get(i);
    ...
    Seq<OlCategory> getCategories(){
        return returnCatArray.clear().addAll(OlCategory.all);
    }

    Seq<Block> getByCategory(OlCategory cat){
        return returnArray.selectFrom(content.blocks(), block ->
                cat.equals(block) && block.isVisible() && block.environmentBuildable());
    }

    Seq<Block> getUnlockedByCategory(OlCategory cat){
    ...
    Block getSelectedBlock(OlCategory cat){
            ...
            full.table(frame -> {
                ...
                //blockCatTable: all blocks | all categories
                {
                    ...
                    blockCatTable.table(categories -> {
                        ...
                        for(OlCategory cat : OlCategory.all){
                        ...
                        int f = 0;
                        for(OlCategory cat : getCategories()){
