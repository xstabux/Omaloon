...
public class MultiBar extends Bar {
    ...
    public void reset(float value) {
        float a = value / barParts.size;

        barParts.each(part -> {
            part.value = part.lastValue = part.blink = a;
        });
    }
    ...
    public void draw() {
        if(this.barParts != null && this.barParts.size > 0) {
            ...
            font.draw(
                    this.name,
                    this.x + this.width / 2.0F - lay.width / 2.0F,
                    this.y + this.height / 2.0F + lay.height / 2.0F + 1.0F
            );
        ...
        public void draw(float x, float y, float width, float height, Seq<BarPart> barParts) {
            if(this.fraction != null) {
                ...
                if (topWidth > (float) Core.atlas.find("bar-top").width) {
                    ...
                    if (ScissorStack.push(scissor.set(this.x, this.y, topWidth, this.height))) {
                        top.draw(
                                this.x,
                                this.y,
                                (float) Core.atlas.find("bar-top").width,
                                this.height
                        );
