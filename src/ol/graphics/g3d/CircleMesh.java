...
public class CircleMesh implements GenericMesh{
    ...
    public void render(PlanetParams params, Mat3D projection, Mat3D transform){
        ...
        OlShaders.planetTextureShader.lightDir.set(planet.solarSystem.position).sub(planet.position).rotate(Vec3.Y, planet.getRotation()).nor();
        OlShaders.planetTextureShader.ambientColor.set(planet.solarSystem.lightColor);

        if(texture == null) texture = new Texture(Core.atlas.getPixmap(Core.atlas.find(texturename)).crop());
        shader.bind();
